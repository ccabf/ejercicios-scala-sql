...
class ClientesDao(var connection: Connection) {

    def createTable: Boolean ={
      val createTableSQL="""create table clients (
    ...
    def save(objeto:ClienteObjeto):ClienteObjeto ={
      ...
      val query: String = " insert into clients (name, tlf,email)" +
        " values (?, ?, ?)"
    ...
    def findAll:List[ClienteObjeto] ={
      val selectSQL="select * from clients;"
    ...
    def update(objeto:ClienteObjeto):ClienteObjeto ={
      val id=objeto.id
      val query: String = " update clients set name=?, tlf=?, email=?" +
        " where id=?"
      ...
      preparedStmt.setString(2, objeto.tlf)
      preparedStmt.setString(3, objeto.email)
      preparedStmt.setInt(4, objeto.id)
      // execute the preparedstatement
    ...
    def delete(id:Int):Boolean={
      ...
      val query: String = " delete from clients  where id=?"
      val preparedStmt: PreparedStatement = connection.prepareStatement(query,Statement.RETURN_GENERATED_KEYS)
